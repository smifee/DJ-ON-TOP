...
if (!player.queue.current.isSeekable) {
		...
		try {
			...
			message.channel.send({embed : {
			  color: "BLACK",
			  description:`<a:LIT_pubg_loading:871120310894755900> Speed is ${player.speed}`}});
		} catch (err) {
  	...
  	if (args[0].toLowerCase() == 'reset' || args[0].toLowerCase() == 'off') {
		player.clearEffects()
		const msg = await message.channel.send(`<a:LIT_pubg_loading:871120310894755900>Turning off **speed**.`);
