...
if (isNaN(args[0])) return message.channel.send('Amount must be a real number.');
		...
		const msg = await message.channel.send(`<a:LIT_pubg_loading:871120310894755900>Setting pitch to **${args[0]}**.`);
 	...
 	if (args[0].toLowerCase() == 'reset' || args[0].toLowerCase() == 'off') {
		player.clearEffects()
		const msg = await message.channel.send(`<a:LIT_pubg_loading:871120310894755900>Turning off **Pitch**.`);
